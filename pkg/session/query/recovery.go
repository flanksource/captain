...
func (q *Loader) recoverMessages(
	...
	transcript, err := q.db.GetTranscriptSession(ctx, overview.ID)
	...
	if transcript == nil {
		transcript, err = q.db.CreateOrGetSession(ctx, database.CreateSessionInput{
			...
			Path: path, CWD: stringValue(overview.CWD),
		})
		if err != nil {
			return nil, fmt.Errorf("bind recovered provider history: %w", err)
		}
	}
	if err := q.registerRecoveredSource(ctx, transcript.ID, source, providerID, path); err != nil {
		return nil, err
	}
...
func needsProviderHistory(overview database.SessionOverview, turns []database.SessionTurn, messages []session.Message) bool {
	if stringValue(overview.ProviderSessionID) == "" {
		return false
	}
...
func (q *Loader) registerRecoveredSource(
	ctx context.Context,
	sessionID uuid.UUID,
	...
	return q.db.RegisterSessionSource(ctx, sessionID, database.IngestSourceInput{
