...
type SpendReader interface {
	BudgetSpendUSD(context.Context, uuid.UUID, map[string]string, time.Time) (float64, error)
}
...
func enforceModel(ctx context.Context, rules []Rule, dimensions map[string]string, model api.Model, now time.Time, spend SpendReader) ([]Attribution, error) {
	...
	for _, rule := range rules {
		...
		if err != nil {
			return nil, fmt.Errorf("read settled spend for budget rule %q: %w", rule.Name, err)
		}
		if used >= rule.Amount {
