...
func TestEffortConfig(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			provider: Anthropic, mode: ModeAPI, model: "claude-sonnet-5", effort: EffortNone,
			want: map[string]any{"max_tokens": 4096},
		},
		{
			name:     "anthropic enabled model uses budget_tokens schema",
			provider: Anthropic, mode: ModeAPI, model: "claude-sonnet-4-6", effort: EffortHigh,
			want: map[string]any{
		...
		},
		{
			...
			provider: Anthropic, mode: ModeAPI, model: "claude-sonnet-5", effort: EffortHigh,
			want: map[string]any{
				"max_tokens":    24576 + 4096,
		...
		},
		{
			...
			provider: Anthropic, mode: ModeAPI, model: "claude-opus-4-8", effort: EffortHigh,
			want: map[string]any{
				"max_tokens":    24576 + 4096,
				"thinking":      map[string]any{"type": "adaptive"},
				"output_config": map[string]any{"effort": "high"},
		...
		},
		{
			...
			provider: Anthropic, mode: ModeAPI, model: "anthropic/claude-fable-5", effort: EffortXHigh,
			want: map[string]any{
		...
		},
		{
			...
			provider: Anthropic, mode: ModeAPI, model: "claude-sonnet-4-6", effort: EffortMedium, maxTokens: 1000,
			want: map[string]any{
		...
		},
		{
			...
			provider: Anthropic, mode: ModeAPI, model: "claude-3-5-sonnet-20241022", effort: EffortHigh,
			want: map[string]any{"max_tokens": 4096},
		},
		{
			name:     "openai reasoning model sets reasoning_effort",
			provider: OpenAI, mode: ModeAPI, model: "gpt-5.5", effort: EffortHigh,
			want: map[string]any{"reasoning_effort": "high"},
		},
		{
			name:     "openai gpt-5.6 passes max effort through",
			provider: OpenAI, mode: ModeAPI, model: "gpt-5.6", effort: EffortMax,
			want: map[string]any{"reasoning_effort": "max"},
		},
		{
			name:     "openai no effort omits config",
			provider: OpenAI, mode: ModeAPI, model: "gpt-5.5", effort: EffortNone,
			want: nil,
		},
		{
			name:     "gemini low effort sets thinkingBudget",
			provider: Google, mode: ModeAPI, model: "gemini-3.5-flash", effort: EffortLow,
			want: map[string]any{"thinkingConfig": map[string]any{"thinkingBudget": 2048}},
		},
		{
			name:     "deepseek omits effort config",
			provider: DeepSeek, mode: ModeAPI, model: "deepseek-v4-pro", effort: EffortHigh,
			want: nil,
		},
		{
			name:     "temperature included for a temperature-capable model",
			provider: Google, mode: ModeAPI, model: "gemini-3.5-flash", effort: EffortNone, temperature: f64(0.5),
			want: map[string]any{"temperature": 0.5},
		},
		{
			name:     "temperature dropped for a temperature-incapable model",
			provider: Anthropic, mode: ModeAPI, model: "claude-sonnet-5", effort: EffortNone, temperature: f64(0.7),
			want: map[string]any{"max_tokens": 4096},
		},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got := EffortConfig(tc.provider, tc.mode, tc.model, tc.effort, tc.maxTokens, tc.temperature)
