...
func TestDeclaredPostureMatchesInitializeParams(t *testing.T) {
	caps := api.PermissionCapabilitiesFor(api.BackendClaudeAgent)
	p := &Provider{}
	for _, mode := range api.AllPermissionModes() {
		t.Run(string(mode), func(t *testing.T) {
			...
			params := p.initializeParams(ai.Request{Permissions: api.Permissions{Mode: mode}})
...
func TestDeclaredDefaultPostureIsSentExplicitly(t *testing.T) {
	declared := api.PermissionCapabilitiesFor(api.BackendClaudeAgent).
	...
	params := (&Provider{}).initializeParams(ai.Request{})
...
func TestDeclaredCallerToolBrokerMatchesApprovalMode(t *testing.T) {
	support := api.PermissionCapabilitiesFor(api.BackendClaudeAgent).
		ToolPolicySupport(api.ProvenanceCaller, api.ToolPolicyAsk)
	require.Equal(t, api.SupportRequiresBroker, support.Kind)

	unbrokered := (&Provider{}).initializeParams(ai.Request{})
	assert.Equal(t, "auto", unbrokered.ApprovalMode, "without a broker there is nothing to ask")

	brokered := (&Provider{cfg: ai.Config{CanUseTool: func(context.Context, ai.PermissionRequest) (ai.PermissionDecision, error) {
...
func TestDeclaredMCPDisableIsRefusedNotDropped(t *testing.T) {
	require.Equal(t, api.SupportUnsupported,
		api.PermissionCapabilitiesFor(api.BackendClaudeAgent).
