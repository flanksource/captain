...
func (db *DB) modelUsageSince(ctx context.Context, since time.Time, schema string) (ModelUsageTotals, bool, error) {
	...
	err = db.gorm.WithContext(ctx).
		...
		Select(`COALESCE(SUM(input_tokens + output_tokens + reasoning_tokens + cache_read_tokens + cache_write_tokens), 0) AS total_tokens,
			COALESCE(SUM(CASE WHEN upper(currency) = 'USD' THEN
				CASE WHEN provider_cost_usd > 0 THEN provider_cost_usd
				ELSE input_cost + output_cost + reasoning_cost + cache_read_cost + cache_write_cost END
